...
//						escribir vector(i) sin saltar
			...
			FinPara
					...
					espacioizq=(posicion)-posicion1
						...
						si espacioizq<=espacioder Entonces     //hola mundo cruel  
							Para i<-posicion+2 Hasta Longitud(frase) con paso 1 Hacer
								vector(i)=subcadena(frase,i-1,i-1)
							Fin Para
							vector(posicion+1)=nuevocaracter
//							Escribir "El espacio vacio más cercano esta a la izquierda por lo tanto se desplazan " espacioizq " valores"
						SiNo
							Para i<-posicion Hasta Longitud(frase) con paso 1 Hacer
								vector(i)=subcadena(frase,i-1,i-1)
							Fin Para
							vector(posicion)=nuevocaracter
